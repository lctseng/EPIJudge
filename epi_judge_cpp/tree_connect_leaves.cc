...
void CreateListOfLeavesHelper(const unique_ptr<BinaryTreeNode<int>> &tree) {
  ...
  if (!tree->left && !tree->right) {
    ...
    CreateListOfLeavesHelper(tree->left);
    CreateListOfLeavesHelper(tree->right);
  }
}

vector<const unique_ptr<BinaryTreeNode<int>> *>
CreateListOfLeaves(const unique_ptr<BinaryTreeNode<int>> &tree) {
  res.clear();
  CreateListOfLeavesHelper(tree);
  return move(res);
}
vector<int>
